...
%Move these part to Chapter 2 !!! Introduce how to generate the spectra step by step !!! Show spectra graph as example. Maybe can use \ref{fig:4.1}, \ref{fig:4.2},\ref{fig:4.3} and \ref{fig:4.4}.
...
\section{Experiments}
%To start with, only four candidates are considered in the first group of experiments. They are all Methionine candidates, with differences in $tilt$ angle, which is the $\theta$ degree. These four candidates each has $\theta$ with the following degree: $0^{\circ}$, $20^{\circ}$,$40^{\circ}$ and $60^{\circ}$. The difference in $\theta$ degree is $20^{\circ}$. As Table \ref{tab:4.1} shows, from Experiment 1 to 4, the target spectra consists of 10\% $\theta = 0^{\circ}$, 50\% $\theta = 20^{\circ}$, 40\% $\theta = 40^{\circ}$ and 60\% $\theta = 40^{\circ}$. For Experiment 1, data points are selected from  x-polarized IR spectrum. For Experiment 2, data points are selected from z-polarized IR spectrum. For Experiment 3, data points are selected from both x- and z-polarized IR spectra. Experiment 4, data points are selected from x-polarized IR spectrum and xx-projection Raman spectrum. Only Experiment's return composition actually matches to the target composition.
...
%This indicates that LP model constructed by using only IR information have achieved its best capacity. However, it is still not sufficient enough to study a case with the current setup. \\

%Should we try LP model with only Raman information(1 projection, 2 projection)??? \\

%And because our candidates are "quite" similar with each other, the return composition from first three experiments can perfectly construct the target spectrum. This also indicates that we need to bring in more information to build the constraints for the LP model. The more constraints we introduce, the more accurate the return composition will be.
...
%10 & 200(irx)+ 200(ramanxx) & [0, 0.653209, 0.33473, 0.0120615]  \\
%\hline
%11 & 200(irx)+ 200(ramanxy) & [0.1, 0.5, 0.4, 0]  \\
%\hline
%12 & 200(irz)+ 200(ramanxx) & [0.1, 0.5, 0.4, 0]  \\
%\hline
%13 & 200(irz)+ 200(ramanxy) & [0.1, 0.5, 0.4, 0]  \\
%\hline
%14 & 200(ramanxx)+ 200(ramanxy) & [0, 0.653209, 0.33473, 0.0120615] \\
%\hline
\end{tabular} 
\end{center}
...
%E 6: combine spectra of IR x projection and Raman xx projection,(Is it possible because they are both in x projection, therefore, the single may not be distinguish enough?)\\
%E 7, 8 and 10: return target composition.\\
%E 9: indicate only using only Raman's two projection's spectra, the information may be limited.\\
%E from 11 to 14: show that IR and Raman, or Raman itself is still sufficient to get the target composition.\\


%For tomorrow: discuss the case when IR, Raman and SFG are not sufficient any more.
%Rerun the following experiments(Because of the SFG did not use imag part, maybe only need to run the ones have sfg)
...
%17 & 500(irx) + 500(irz) + 500(ramanxx) + 500(ramanxy) + 500(ramanxz) + 500(ramanzz) + 500(sfgyyz) + 500(sfgyzy) + 500(sfgzzz) & [0.321014, 0, 0.31018, 0.163041, 0.205764]  \\ \hline
%18 & 1000(irx) + 1000(irz) + 1000(ramanxx) + 1000(ramanxy) + 1000(ramanxz) + 1000(ramanzz) + 1000(sfgyyz) + 1000(sfgyzy) + 1000(sfgzzz) & [0.321014, 0, 0.31018, 0.163041, 0.205764]  \\ \hline
\end{tabular} 
\end{center}
\caption{Experiment 5 to Experiment 9 Setting for Methionine Candidates}
\end{table}
...
%Do we need to conduct more experiments to derive the following conclusion???
%Possible experiment suggestions:
%expand the $\theta$ to from $0^{\circ}$ to $90^{\circ}$, randomly select three candidates to generate the target composition. if it works, then four candidates; till it fails.
%expand the $\theta$ to from $0^{\circ}$ to $180^{\circ}$, randomly select three candidates to generate the target composition. if it works, then four candidates; till it fails.

%These experiments already indicate that, when we try to study the molecular coordination distribution for one kind of molecule at surfaces, even combining the information from all three spectra techniques, it is not sufficient to derive the target composition of the coordination distribution.
...
%In this experiment, we use Methionine as an example, and generate a random target composition as List \ref{eqn: 4.1}. In this list, there are $9$ candidates of methionine with $\theta$ of $0^{\circ}$, $10^{\circ}$, $20^{\circ}$, $30^{\circ}$, $40^{\circ}$, $50^{\circ}$, $60^{\circ}$, $70^{\circ}$, and $80^{\circ}$. Then I randomly select five candidates to be the present in the composition, and the rest candidates do not exist in the target composition. Then I will use this experiment as an example to explain why the composition returned by our LP model is different from the target composition.  


%From the previous experiments, we know that even combining all three spectroscopy techniques is not sufficient enough to obtain the target composition. Therefore, we will combine these three techniques to obtain the maximum amount of information to further study our LP model, to illustrate why the LP model fail to obtain the target composition. In the following experiments, I am going to select different amount of data points to build LP models, each model returns different composition as shown in List \ref{eqn: 4.2} and \ref{eqn: 4.3}. List \ref{eqn: 4.2} is a composition return by the LP model where data points are evenly selected by every 5 wave length along all the spectra of the three techniques.  For list \ref{eqn: 4.3}, the data points are evenly selected every 500 wave length. \\
 
%Target Composition
%\begin{equation} \label{eqn: 4.1}
%[0.2201, 0.28905, 0.05201, 0.08251, 0.35633, 0, 0, 0, 0]
%\end{equation} 

%$probe_arrange = arange(1000,2000,5)$
%Return Composition when data points are selected each $5$ wave length
%\begin{equation} \label{eqn: 4.2}
%[0.158921, 0.388434, 0.0, 0.0985466, 0.354099, 0.0, 0.0, 0.0, 0.0]
%\end{equation}

%\begin{equation} \label{eqn: 4.3}
%[0.161666, 0.385035, 0.0, 0.0995971, 0.353696, 0.0, 0.0, 0.0, 6.5841e-06]
%\end{equation}  \caption{Return Composition when data points are selected each $200$ wave length}

%Return Composition when data points are selected each $500$ wave length
%$probe_arrange = arange(1000,2000,500)$
%\begin{equation} \label{eqn: 4.3}
%[0.397991, 0.0, 0.203394, 0.0357663, 0.362848, 0.0, 0.0, 0.0, 0.0]
%\end{equation} 
...
%When LP does not have unique solution? Different data point selection may result in different return compositions, but the spectra constructed by all these return compositions may be identical or similar to target spectra. Quality of LP solutions of different sample inputs for the same candidates are almost identical, but the solutions themselves are very different from each other. Run experiments, and prove.

%The LP model is right. LP model with spectroscopy input, 

%It is the data problem, not LP problem.

%Any (\%) composition of will result in same/almost same target spectra , this would not be the same case for very different candidates.



%What if we have exact same candidates of one molecule, will LP solution be unbounded? 
